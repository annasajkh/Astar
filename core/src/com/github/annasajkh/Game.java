...
public class Game extends ApplicationAdapter
{
    ...
    public void create()
    {
        ...
        for (int i = 0; i < rows; i++)
        {
            ...
            {
                nodes[i][j] = new Node(new Vector2(j * resolution + resolution / 2f, i * resolution +
                                                                                     resolution /
                                                                                     2f), new Size(resolution, resolution), Color.WHITE, i, j);
                nodes[i][j].centered = true;
            }

        }

        start = nodes[random.nextInt(rows)][random.nextInt(columns)];
        end = nodes[random.nextInt(rows)][random.nextInt(columns)];
        start.start = true;
        end.end = true;
        openSet.add(start);
